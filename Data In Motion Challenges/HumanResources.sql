...
SELECT department_id ,name, DATEDIFF(end_date,start_date ) as ProjectDuration
FROM projects
ORDER BY ProjectDuration;
...
SELECT e.name, e.hire_date, e.department_id, p.name, p.start_date
FROM employees e 
...
dense_rank() over (PARTITION BY department_id order by hire_date) as Ranking
FROM employees;
...
SELECT
    ...
    department_id,
    DATEDIFF(
        LEAD(hire_date) OVER (PARTITION BY department_id ORDER BY hire_date),
        hire_date
    ) AS duration_in_days
FROM
    employees 
